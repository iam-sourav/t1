...
try {
  ...
  _(".lg_name")[0].innerText = project_data.all_games[`g${allGamesLength - 1}`].name;
  _(".lg_v")[0].innerText = "Version :" + project_data.all_games[`g${allGamesLength - 1}`].version; 
  ...
  function updateIMG() {
    setTimeout(() => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        addSLC.style.backgroundImage = `url("resource/imgs/g${allGamesLength - 1}/p${imgArray[liveIndex][i]}.jpg")`;
      }
...
function setTab(i) {
  try {
    ...
    const h2 = ce("h2", game_details, [], project_data.all_games[`g${i}`].name); 
    const h6 = ce("h6", game_details, [], project_data.all_games[`g${i}`].details); 
    // image box
    ...
    ce("h6", gmabout, [], "Version : " + project_data.all_games[`g${i}`].version);
    // play button
...
try {
  for (let i = 0; i < allGamesLength - 1; i++) {  
    ...
    const h5 = ce("h6", game_dtls, [], project_data.all_games[`g${i}`].details);
  }
...
function makeCard(TargatElement, i, classindex) {
  try {
    ...
    const nameP = ce("p", card_name, [], project_data.my_creation[`p${i}`].name); 
    const language = ce("div", card_dtls, ["language"]);
    for (let j = 0; j <= project_data.my_creation[`p${i}`].languases.length; j++) {
...
try {
  ...
  for (let i = myAppsLength - 1; i >= 0; i--) {
    ...
    const pn = ce("h4", app_name, [], project_data.my_apps[`app${i}`].name + " \n " + project_data.my_apps[`app${i}`].game_type);
    const app_from = ce("div", adc, ["app_from"]);
    const af = ce("h5", app_from, [], project_data.my_apps[`app${i}`].platform);
    const app_pack = ce("div", adc, ["app_pack"]);
    const pp = ce("p", app_pack, [], project_data.my_apps[`app${i}`].game_graphics);
    const app_dtls = ce("div", adc, ["app_dtls"]);
    const prd = ce("p", app_dtls, [], project_data.my_apps[`app${i}`].package_name);
    const app_v = ce("div", adc, ["app_v"]);
    const pv = ce("p", app_v, [], "Version " + project_data.my_apps[`app${i}`].version); 
  }
} catch (err) {
  console.log(err);
}
...
function appDT(i) {
  try {
    ...
    const aname = ce("h2", an, [], project_data.my_apps[`app${i}`].name + " \n " + project_data.my_apps[`app${i}`].game_type);
    const ap = ce("div", app_tab_details, ["apps"]);
    const apack = ce("p", ap, [], project_data.my_apps[`app${i}`].package_name);
    ...
    for (
      let j = 0; j < project_data.my_apps[`app${i}`].images.length; j++) { 
      let size = parseInt(project_data.my_apps[`app${i}`].images[j]) == 1 ? `${size_big}px` : `${size_big / 3}px`;
    ...
    const v_num = ce("div", app_ad, ["v_num"], project_data.my_apps[`app${i}`].version);
    const releas = ce("div", app_ad, ["releas"], "Released on");
    const releas_date = ce("div", app_ad, ["releas_date"], project_data.my_apps[`app${i}`].released_date);
...
function projectClickScroll(dir) {
  ...
  function loop() {
    ...
    if ((everyProject.scrollLeft + 10) >= everyProject.scrollWidth - everyProject.clientWidth)
      slide_btn[1].style.width = ac([_("#slide_b2")]);
    else slide_btn[1].style.width = rc([_("#slide_b2")]);
    if (everyProject.scrollLeft - 5 <= 0) slide_btn[0].style.width = ac([_("#slide_b1")]);
